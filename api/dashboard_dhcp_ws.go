...
// dashboardWSAuthorized checks API token for the WebSocket handshake (query param for browsers).
...
// dashboardWebSocketHandler pushes dashboard JSON when the payload changes (bounded tick).
func dashboardWebSocketHandler(w http.ResponseWriter, r *http.Request, d *Deps) {
	if r.Method != http.MethodGet {
		w.WriteHeader(http.StatusMethodNotAllowed)
		return
	}
	cfg := d.Cfg()
	if cfg == nil || !cfg.StatsDashboardHTMLEnabled() {
		http.NotFound(w, r)
		return
	}
	tok := ""
	if d.AuthToken != nil {
		tok = strings.TrimSpace(d.AuthToken())
	}
